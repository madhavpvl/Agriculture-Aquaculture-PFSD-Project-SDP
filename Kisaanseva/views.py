...
def ws(request):
    wsitems = items.objects.filter(category="Weekend Specials")
...
def delights(request):
    ritems = ritem.objects.all()
    return render(request, 'delights.html', {'ritem': ritem})
...
def add_rec(request):
    if request.method=='POST':
        ...
        rec.save()
        if rec.time=="morning":
            rm=ritem.objects.filter(category="morning")
